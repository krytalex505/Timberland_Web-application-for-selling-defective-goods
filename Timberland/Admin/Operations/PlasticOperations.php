...
if (isset($_GET['action']) && $_GET['action'] == 'getStats') {
    ...
    $totalResult = $connection->query("SELECT COUNT(*) as total FROM plastic");
    $total = $totalResult->fetch_assoc()['total'];
    
    // Активные записи (где заполнена номенклатура)
    $activeResult = $connection->query("SELECT COUNT(*) as active FROM plastic WHERE Nomenclature IS NOT NULL AND Nomenclature != ''");
...
if (isset($_GET['action']) && $_GET['action'] == 'getData') {
    ...
    $countResult = $connection->query("SELECT COUNT(*) as total FROM plastic $where");
    $total = $countResult->fetch_assoc()['total'];
    
    // Получение данных для текущей страницы
    $query = "SELECT * FROM plastic $where ORDER BY id DESC LIMIT $offset, $limit";
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $imageQuery = $connection->query("SELECT image_path FROM images_plastic WHERE plastic_id = $id");
        $images = [];
...
if (isset($_POST['create'])) {
    ...
    $stmt = $connection->prepare("INSERT INTO `plastic` 
    ...
    if ($stmt->execute()) {
        $plastic_id = $stmt->insert_id;

        if (!empty($imagePaths)) {
            $imgStmt = $connection->prepare("INSERT INTO images_plastic (plastic_id, image_path) VALUES (?, ?)"); 
            foreach ($imagePaths as $path) {
                $imgStmt->bind_param("is", $plastic_id, $path);
                $imgStmt->execute();
            }
            $imgStmt->close();
        }

        header("Location: ../List/PlasticTable.php");
        exit();
    } else {
...
if (isset($_GET['delete'])) {
    ...
    $connection->query("DELETE FROM images_plastic WHERE plastic_id = $id");
    $connection->query("DELETE FROM plastic WHERE id = $id");
