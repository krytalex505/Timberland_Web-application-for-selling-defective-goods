...
if (isset($_GET['action']) && $_GET['action'] == 'getStats') {
    ...
    $totalResult = $connection->query("SELECT COUNT(*) as total FROM mdf");
    $total = $totalResult->fetch_assoc()['total'];
    
    // Активные записи (где заполнена номенклатура)
    $activeResult = $connection->query("SELECT COUNT(*) as active FROM mdf WHERE Nomenclature IS NOT NULL AND Nomenclature != ''");
...
if (isset($_POST['create'])) {
    ...
    $stmt = $connection->prepare("INSERT INTO `mdf` 
    ...
    if ($stmt->execute()) {
        $mdf_id = $stmt->insert_id;

        if (!empty($imagePaths)) {
            $imgStmt = $connection->prepare("INSERT INTO images_mdf (mdf_id, image_path) VALUES (?, ?)"); 
            foreach ($imagePaths as $path) {
                $imgStmt->bind_param("is", $mdf_id, $path);
                $imgStmt->execute();
            }
            $imgStmt->close();
        }

        header("Location: ../List/MDFTable.php");
        exit();
    } else {
...
if (isset($_GET['delete'])) {
    ...
    $imageQuery = $connection->query("SELECT image_path FROM images_mdf WHERE mdf_id = $id");
    if ($imageQuery) {
    ...
    $connection->query("DELETE FROM images_mdf WHERE mdf_id = $id");
    $connection->query("DELETE FROM mdf WHERE id = $id");
