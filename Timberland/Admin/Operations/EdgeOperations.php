...
if (isset($_GET['action']) && $_GET['action'] == 'getStats') {
    ...
    $totalResult = $connection->query("SELECT COUNT(*) as total FROM edge");
    $total = $totalResult->fetch_assoc()['total'];
    
    // Активные записи (где заполнена номенклатура)
    $activeResult = $connection->query("SELECT COUNT(*) as active FROM edge WHERE Nomenclature IS NOT NULL AND Nomenclature != ''");
...
if (isset($_POST['create'])) {
    ...
    $stmt = $connection->prepare("INSERT INTO `edge` 
        (Nomenclature, DecorNumber, `Character`, `Complaint`, `Size`, `Basic`,
         `Group`, `Unit`, `accounting`, `Sheets`, `WareHouse`, `Cost`, `Date`, `FromWho`, `DateComplaint`, `Percent`, `Compensation`, `Sum`) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    if ($stmt->execute()) {
        $edge_id = $stmt->insert_id;

        if (!empty($imagePaths)) {
            $imgStmt = $connection->prepare("INSERT INTO images_edge (edge_id, image_path) VALUES (?, ?)"); 
            foreach ($imagePaths as $path) {
                $imgStmt->bind_param("is", $edge_id, $path);
                $imgStmt->execute();
            }
            $imgStmt->close();
        }

        header("Location: ../List/EdgeTable.php");
        exit();
    } else {
...
if (isset($_GET['delete'])) {
    ...
    $imageQuery = $connection->query("SELECT image_path FROM images_edge WHERE edge_id = $id");
    if ($imageQuery) {
    ...
    $connection->query("DELETE FROM images_edge WHERE edge_id = $id");
    $connection->query("DELETE FROM edge WHERE id = $id");
