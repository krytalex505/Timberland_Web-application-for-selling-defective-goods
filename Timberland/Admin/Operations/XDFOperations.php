...
if (isset($_GET['action']) && $_GET['action'] == 'getStats') {
    ...
    $totalResult = $connection->query("SELECT COUNT(*) as total FROM xdf");
    $total = $totalResult->fetch_assoc()['total'];
    
    // Активные записи (где заполнена номенклатура)
    $activeResult = $connection->query("SELECT COUNT(*) as active FROM xdf WHERE Nomenclature IS NOT NULL AND Nomenclature != ''");
...
if (isset($_POST['create'])) {
    ...
    $stmt = $connection->prepare("INSERT INTO `xdf` 
    ...
    if ($stmt->execute()) {
        $xdf_id = $stmt->insert_id;

        if (!empty($imagePaths)) {
            $imgStmt = $connection->prepare("INSERT INTO images_xdf (xdf_id, image_path) VALUES (?, ?)"); 
            foreach ($imagePaths as $path) {
                $imgStmt->bind_param("is", $xdf_id, $path);
                $imgStmt->execute();
            }
            $imgStmt->close();
        }

        header("Location: ../List/XDFTable.php");
        exit();
    } else {
...
if (isset($_GET['delete'])) {
    ...
    $imageQuery = $connection->query("SELECT image_path FROM images_xdf WHERE xdf_id = $id");
    if ($imageQuery) {
    ...
    $connection->query("DELETE FROM images_xdf WHERE xdf_id = $id");
    $connection->query("DELETE FROM xdf WHERE id = $id");
