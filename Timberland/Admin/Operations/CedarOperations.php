...
// В начале файла, после подключения к БД, добавьте:

// Получение статистики (общее количество и активные записи)
if (isset($_GET['action']) && $_GET['action'] == 'getStats') {
    header('Content-Type: application/json');
    
    $totalResult = $connection->query("SELECT COUNT(*) as total FROM cedar");
    $total = $totalResult->fetch_assoc()['total'];
    
    // Активные записи (где заполнена номенклатура)
    $activeResult = $connection->query("SELECT COUNT(*) as active FROM cedar WHERE Nomenclature IS NOT NULL AND Nomenclature != ''");
...
if (isset($_POST['create'])) {
    ...
    $stmt = $connection->prepare("INSERT INTO `cedar` 
    ...
    if ($stmt->execute()) {
        $cedar_id = $stmt->insert_id;

        if (!empty($imagePaths)) {
            $imgStmt = $connection->prepare("INSERT INTO images_cedar (cedar_id, image_path) VALUES (?, ?)"); 
            foreach ($imagePaths as $path) {
                $imgStmt->bind_param("is", $cedar_id, $path);
                $imgStmt->execute();
            }
            $imgStmt->close();
        }

        header("Location: ../List/CedarTable.php");
        exit();
    } else {
...
if (isset($_GET['delete'])) {
    ...
    $imageQuery = $connection->query("SELECT image_path FROM images_cedar WHERE cedar_id = $id");
    if ($imageQuery) {
    ...
    $connection->query("DELETE FROM images_cedar WHERE cedar_id = $id");
    $connection->query("DELETE FROM cedar WHERE id = $id");
