...
if (isset($_GET['action']) && $_GET['action'] == 'getStats') {
    ...
    $totalResult = $connection->query("SELECT COUNT(*) as total FROM countertopsegger");
    $total = $totalResult->fetch_assoc()['total'];
    
    // Активные записи (где заполнена номенклатура)
    $activeResult = $connection->query("SELECT COUNT(*) as active FROM countertopsegger WHERE Nomenclature IS NOT NULL AND Nomenclature != ''");
...
if (isset($_POST['create'])) {
    ...
    $stmt = $connection->prepare("INSERT INTO `countertopsegger` 
    ...
    if ($stmt->execute()) {
        $countertopsegger_id = $stmt->insert_id;

        if (!empty($imagePaths)) {
            $imgStmt = $connection->prepare("INSERT INTO images_countertopsegger (countertopsegger_id, image_path) VALUES (?, ?)"); 
            foreach ($imagePaths as $path) {
                $imgStmt->bind_param("is", $countertopsegger_id, $path);
                $imgStmt->execute();
            }
            $imgStmt->close();
        }

        header("Location: ../List/CountertopsEggerTable.php");
        exit();
    } else {
...
if (isset($_GET['delete'])) {
    ...
    $imageQuery = $connection->query("SELECT image_path FROM images_countertopsegger WHERE countertopsegger_id = $id");
    if ($imageQuery) {
    ...
    $connection->query("DELETE FROM images_countertopsegger WHERE countertopsegger_id = $id");
    $connection->query("DELETE FROM countertopsegger WHERE id = $id");
...
if (isset($_POST['edit'])) {
    ...
    $query = "UPDATE `countertopsegger` SET 
        Nomenclature = ?, 
        DecorNumber = ?, 
        `Character` = ?, 
        `Complaint` = ?, 
