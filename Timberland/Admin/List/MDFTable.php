...
$totalRecordsCount = $connection->query("SELECT COUNT(*) as total FROM mdf")->fetch_assoc()['total'];
?>
<html>
<head>
    <meta charset="utf-8">
...
<div class="table-container">
    <table class="table custom-table" id="dataTable">
        ...
        <tbody>
        <?php
        $result = $connection->query("SELECT * FROM mdf ORDER BY id ASC");
        ...
        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                <tr data-id="<?= $id ?>">
                    ...
                    <td>
                        <div class="btn-group">
                            <a href="../Edit/editMDF.php?edit=<?= $id ?>" class="btn btn-warning btn-sm">Изменить</a>
                            <a href="../Operations/MDFOperations.php?delete=<?= $id ?>" class="btn btn-danger btn-sm btn-delete">Удалить</a>
                        </div>
                    </td>
                </tr>
                <?php
...
<div class="modal fade" id="createWindow" tabindex="-1" aria-labelledby="createModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="MDFForm" method="POST" action="../Operations/MDFOperations.php" enctype="multipart/form-data">
                <div class="modal-header">
...
document.getElementById('MDFForm').addEventListener('submit', function(e) {
    e.preventDefault();
