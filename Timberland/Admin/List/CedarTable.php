...
$totalRecordsCount = $connection->query("SELECT COUNT(*) as total FROM cedar")->fetch_assoc()['total'];
?>
<html>
<head>
    <meta charset="utf-8">
...
<div class="table-container">
    <table class="table custom-table" id="dataTable">
        ...
        <tbody>
        <?php
        $result = $connection->query("SELECT * FROM cedar ORDER BY id ASC");
        ...
        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                <tr data-id="<?= $id ?>">
                    ...
                    <td>
                        <div class="btn-group">
                            <a href="../Edit/editCedar.php?edit=<?= $id ?>" class="btn btn-warning btn-sm">Изменить</a>
                            <a href="../Operations/CedarOperations.php?delete=<?= $id ?>" class="btn btn-danger btn-sm btn-delete">Удалить</a>
                        </div>
                    </td>
                </tr>
                <?php
...
<div class="modal fade" id="createWindow" tabindex="-1" aria-labelledby="createModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="CedarForm" method="POST" action="../Operations/CedarOperations.php" enctype="multipart/form-data">
                <div class="modal-header">
...
document.getElementById('CedarForm').addEventListener('submit', function(e) {
    e.preventDefault();
