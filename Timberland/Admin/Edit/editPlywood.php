...
$stmt = $connection->prepare("SELECT * FROM plywood WHERE Id=?");
...
if (isset($_GET['delete_image']) && is_numeric($_GET['delete_image'])) {
    ...
    $imageQuery = $connection->prepare("SELECT image_path FROM images_plywood WHERE id = ?"); 
    ...
    if ($imageResult->num_rows > 0) {
        ...
        $deleteImageQuery = $connection->prepare("DELETE FROM images_plywood WHERE id = ?");
        $deleteImageQuery->bind_param("i", $image_id);
        $deleteImageQuery->execute();

        header("Location: editPlywood.php?edit=" . $id);
        exit();
    }
}
?>
...
<html lang="ru">
<head>
    <title>Редактирование записи - Фанера</title>
...
<div class="container mt-4">
    <h3>Редактирование записи - Фанера</h3>
    <form id="editPlywood" method="POST" action="../Operations/PlywoodOperations.php" onsubmit="return confirmEdit();" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <div id="currentImages" class="d-flex flex-wrap">
                <?php
                $imageQuery = $connection->query("SELECT id, image_path FROM images_plywood WHERE plywood_id = $id");
                ...
                ?>
                    <div class="image-container text-center m-2">
                        ...
                        <br>
                        <a href="editPlywood.php?edit=<?= $id ?>&delete_image=<?= $img['id'] ?>" class="btn btn-danger btn-sm mt-1" onclick="return confirm('Вы уверены, что хотите удалить это изображение?');">Удалить</a>
                    </div>
                <?php 
                    endwhile;
                else: 
        ...
        <div class="form-group text-center">
            ...
            <a href="../List/PlywoodTable.php" class="btn btn-secondary">Отмена</a>
        </div>
    </form>
</div>
...
// Отправка формы
document.getElementById('editPlywood').addEventListener('submit', function(e) {
    e.preventDefault();
