...
$stmt = $connection->prepare("SELECT * FROM egger WHERE Id=?");
...
if (isset($_GET['delete_image']) && is_numeric($_GET['delete_image'])) {
    ...
    $imageQuery = $connection->prepare("SELECT image_path FROM images_egger WHERE id = ?"); 
    ...
    if ($imageResult->num_rows > 0) {
        ...
        $deleteImageQuery = $connection->prepare("DELETE FROM images_egger WHERE id = ?");
        $deleteImageQuery->bind_param("i", $image_id);
        $deleteImageQuery->execute();

        header("Location: editEgger.php?edit=" . $id);
        exit();
    }
}
?>
...
<html lang="ru">
<head>
    <title>Редактирование записи - Эггер ДСП</title>
...
<div class="container mt-4">
    <h3>Редактирование записи - Эггер ДСП</h3>
    <form id="editEgger" method="POST" action="../Operations/EggerOperations.php" onsubmit="return confirmEdit();" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <div id="currentImages" class="d-flex flex-wrap">
                <?php
                $imageQuery = $connection->query("SELECT id, image_path FROM images_egger WHERE egger_id = $id");
                ...
                ?>
                    <div class="image-container text-center m-2">
                        ...
                        <br>
                        <a href="editEgger.php?edit=<?= $id ?>&delete_image=<?= $img['id'] ?>" class="btn btn-danger btn-sm mt-1" onclick="return confirm('Вы уверены, что хотите удалить это изображение?');">Удалить</a>
                    </div>
                <?php 
                    endwhile;
                else: 
        ...
        <div class="form-group text-center">
            ...
            <a href="../List/EggerTable.php" class="btn btn-secondary">Отмена</a>
        </div>
    </form>
</div>
...
// Отправка формы
document.getElementById('editEgger').addEventListener('submit', function(e) {
    e.preventDefault();
