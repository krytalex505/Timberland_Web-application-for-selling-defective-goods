...
$id = intval($_GET['edit']); // Приводим к числу

// Получаем данные из базы по ID
$stmt = $connection->prepare("SELECT * FROM byspan WHERE Id=?");
...
$row = $result->fetch_assoc(); // Данные из базы
...
if (isset($_GET['delete_image']) && is_numeric($_GET['delete_image'])) {
    ...
    $imageQuery = $connection->prepare("SELECT image_path FROM images_byspan WHERE id = ?"); 
...
<div class="container mt-4">
    ...
    <form id="editForm" method="POST" action="../Operations/BySpanOperations.php" onsubmit="return confirmEdit();" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <!-- Зона для вставки из буфера обмена -->
        ...
        <!-- ТЕКУЩИЕ ИЗОБРАЖЕНИЯ (без кнопки копирования) -->
        <div class="form-group">
            <label class="mb-2">Текущие изображения:</label>
            <div id="currentImages" class="d-flex flex-wrap">
                <?php
                $imageQuery = $connection->query("SELECT id, image_path FROM images_byspan WHERE byspan_id = $id");
                $allImages = [];
                if ($imageQuery && $imageQuery->num_rows > 0):
                    while ($img = $imageQuery->fetch_assoc()):
                        ...
                        $allImages[] = $imagePath;
                ?>
                    <div class="image-container text-center m-2" data-image-path="<?= $imagePath ?>">
                        ...
                        <br>
                        <a href="editBySpan.php?edit=<?= $id ?>&delete_image=<?= $img['id'] ?>" class="btn btn-danger btn-sm mt-1" onclick="return confirm('Вы уверены, что хотите удалить это изображение?');">Удалить</a>
                    </div>
                <?php 
                    endwhile;
                else: 
        ...
        <div class="form-group text-center">
            ...
            <a href="../List/BySpanTable.php" class="btn btn-secondary">Отмена</a>
        </div>
    </form>
</div>

<!-- Подключаем jQuery и Fancybox JS -->
...
// Отображение списка файлов с превью
...
<!-- Уведомление Bootstrap -->
<div id="alert-box" class="alert alert-danger alert-dismissible fade show d-none"
     role="alert"
